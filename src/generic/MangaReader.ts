...
} from "./MangaReaderHelper";
import { MangaReaderInterceptor } from "./MangaReaderInterceptor";
import {
    MangaReaderDiscoverSection,
    MangaReaderSearchMetadata,
    MangaReaderSlug,
    Months,
    StatusTypes,
} from "./MangaReaderInterfaces";
import { MangaReaderParser } from "./MangaReaderParser";
import { getUsePostIds, MangaReaderSettings } from "./MangaReaderSettingsForm";

export abstract class MangaReaderGeneric
    implements
        Extension,
        SearchResultsProviding,
        MangaProviding,
        ...
        CloudflareBypassRequestProviding
{
    ...
    featuredSection: MangaReaderDiscoverSection = {
        id: "popular",
        title: "Popular Today",
    ...
    latestUpdatesSection: MangaReaderDiscoverSection = {
        id: "latest_updates",
    ...
    discoverSections: MangaReaderDiscoverSection[] = [
    ...
    setup() {
        this.requestManager = new MangaReaderInterceptor("main", this.domain);
    }
    ...
    async getSearchResults(
        query: SearchQuery,
        metadata: MangaReaderSearchMetadata | undefined,
    ...
    async getDiscoverSections(): Promise<DiscoverSection[]> {
        return this.discoverSections.map((x: MangaReaderDiscoverSection) => {
            return {
                id: x.id,
                subtitle: x.subtitle,
                type: x.type,
    ...
    async getDiscoverSectionItems(
        ...
        this.checkResponseError(request, response);
        const $ = cheerio.load(Application.arrayBufferToUTF8String(buffer));
        let s: MangaReaderDiscoverSection;
        switch (section.id) {
            case "featured":
            case "popular":
    ...
    async getSettingsForm(): Promise<Form> {
        return new MangaReaderSettings(this.name);
    }
    ...
    async convertPostIdToSlug(postId: number): Promise<MangaReaderSlug> {
        const request = {
